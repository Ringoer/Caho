...
const defaultAvatarUrl =
  'https://static.ringoer.com/cdn/caho/avatar/default_forum_avatar.png';
...
export default connect(
  ...
  useEffect(() => {
    props.dispatch({
      ...
      payload: [{ index: 1, pathname: '/forum/add', name: '[版块] 新建版块' }],
    });
  }, []);
  useEffect(() => {
    if (!flag) {
    ...
    Swal.confirm('您确定要提交申请吗？').then((res) => {
      if (!res) {
        return;
      }
      ...
      {user ? (
        ...
        >
          <div className={styles.wrapper}>
            ...
            />
            {state !== 0 && !(forumName.current && forumName.current.value) ? (
              <label htmlFor="forumName" className={styles.warning}>
                版块名称不能为空！
              </label>
            ) : undefined}
          </div>
          <div className={styles.wrapper}>
            ...
            !(description.current && description.current.value) ? (
              <label htmlFor="description" className={styles.warning}>
                版块介绍不能为空！
              </label>
            ) : undefined}
          </div>
